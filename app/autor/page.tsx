  ...
  return (
    <ScreenShell>
      <ScreenHero
        ...
        intro={`${author?.nationality ? `${author.nationality}. ` : ""}${books.length} ${books.length === 1 ? "título publicado" : "títulos publicados"} en Literatura SM.`}
      />
      <section className="author-profile">
        <div className="author-profile-mark">{initials(author?.name || "A")}</div>
        <div>
          <p className="eyebrow">Trayectoria en el catálogo</p>
          <h2>Historias para descubrir<br /><em>nuevas miradas.</em></h2>
          <p>Explora sus títulos, colecciones, edades recomendadas y recursos disponibles.</p>
          <Link className="arrow-link" href="/recursos">Ver recursos de lectura ↗</Link>
        </div>
      </section>
